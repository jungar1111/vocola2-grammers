...
restart session = Keys.SendInput({alt+space}) Wait(100) Keys.SendInput("r") {enter};backup
...
#linux commands
jungar | root | root@ | msd.local | data.host | .14 | .214 | 10.1.1.181 | .28 | msdsyslog :
...
#IOS
#Nexus	
#10. | 192. | 172. | 216. | 8. | .173 | fo-isp | 65. | 208. | 24. | 50. | 96. | 75. ;
#10. | 192. | 172. | 216. | 8. | .173 | 65. | 208. | 207. | 24. | 50. | 96. | 75. | 12. | -  :
10. | 192. | 172. | 216. | 8. | .174 | .173 | 65. | 208. | 207. | 24. | 50. | 96. | 75. | 12. | 155. | 166. :
	...
	#	interface <intnameNexus> <n> <n> <n> slash <n> slash <n> = "interface " $1 EvalTemplate($2) EvalTemplate($3) EvalTemplate($4) "/" EvalTemplate($5) "/" EvalTemplate($6);
	#	interface <intnameNexus> <n> <n> <n> slash <n> slash <n> dash <n> = "interface " $1 #EvalTemplate($2) EvalTemplate($3) EvalTemplate($4) "/" EvalTemplate($5) "/" EvalTemplate($6) "-" #EvalTemplate($7);
	#interface <intnameNexus> <n> <n> <n> = "interface " $1 EvalTemplate($2) "/" EvalTemplate($3) "/" EvalTemplate($4);
	
	#	Vocola nexus = "here";
#	<intnameNexus> := 'e' | 'Ethernet' = e | port channel = 'port-channel' ;
#	interface <intnameNexus> <n> = "interface " $1 EvalTemplate($2);
#	interface <intnameNexus> <n> <n> <n> = "interface " $1 EvalTemplate($2) "/" EvalTemplate($3) "/" EvalTemplate($4);
#	interface <intnameNexus> <n> <n> <n> slash <n> slash <n> = "interface " $1 EvalTemplate($2) EvalTemplate($3) EvalTemplate($4) "/" EvalTemplate($5) "/" EvalTemplate($6);
#	interface <intnameNexus> <n> <n> <n> slash <n> slash <n> dash <n> = "interface " $1 EvalTemplate($2) EvalTemplate($3) EvalTemplate($4) "/" EvalTemplate($5) "/" EvalTemplate($6) "-" EvalTemplate($7);
...
#
	...
	no <firstword> <secondword> pause = "no $1 $2 ";
	no <firstword> <secondword> <thirdword> = "no $1 $2 $3{Enter}";
	no <firstword> <secondword> <thirdword> pause = "no $1 $2 $3";
	no <firstword> <secondword> <n> = "no $1 $2 $3{Enter}";
	no <firstword> <secondword> <n> pause = "no $1 $2 $3 ";
	no <firstword> <secondword> <thirdword> <fourthword> = "no $1 $2 $3 $4{Enter}";
	no <firstword> <secondword> <thirdword> <fourthword> pause = "no $1 $2 $3 $4 ";
	no <firstword> <secondword> <n> <fourthword> = "no $1 $2 $3 $4{Enter}";
	no <firstword> <secondword> <n> <fourthword> pause = "no $1 $2 $3 $4 ";
	no <firstword> <secondword> <thirdword> <fourthword> <fifthword> = "no $1 $2 $3 $4 $5{Enter}";
	no <firstword> <secondword> <thirdword> <fourthword> <fifthword> pause = "no $1 $2 $3 $4 $5";
	<firstword> = "$1 ";
	<firstword> <secondword> = "$1 $2{Enter}";
	<firstword> <secondword> pause =  "$1 $2 ";
	<firstword> <secondword> <thirdword> = "$1 $2 $3{Enter}";
	<firstword> <secondword> <thirdword> pause =  "$1 $2 $3 ";
	<firstword> <secondword> <n> = "$1 $2 $3{Enter}";
	<firstword> <secondword> <n> pause =  "$1 $2 $3 ";
	<firstword> <secondword> <thirdword> <fourthword> = "$1 $2 $3 $4";
	<firstword> <secondword> <thirdword> <fourthword> pause =  "$1 $2 $3 $4 ";
	<firstword> <secondword> <n> <fourthword> = "$1 $2 $3 $4{Enter}";
	<firstword> <secondword> <n> <fourthword> pause =  "$1 $2 $3 $4 ";
	<firstword> <secondword> <thirdword> <fourthword> <fifthword> = "$1 $2 $3 $4 $5{Enter}";
	<firstword> <secondword> <thirdword> <fourthword> <fifthword> pause =  "$1 $2 $3 $4 $5 ";
	<firstword> <secondword> <n> <fourthword> <fifthword> = "$1 $2 $3 $4 $5{Enter}";
	<firstword> <secondword> <n> <fourthword> <fifthword> pause =  "$1 $2 $3 $4 $5 in new dog grammar's ";
	<firstword> <secondword> into <intname> <n> = "$1 $2 int $3$4 {Enter}";
	<firstword> <secondword> into <intname> <n> pause =  "$1 $2 int $3$4";
	<firstword> <secondword> into <intname> <n> <n> = "$1 $2 int $3$4/$5{Enter}";
	<firstword> <secondword> into <intname> <n> <n> pause =  "$1 $2 int $3$4/$5";
	<firstword> <secondword> into <intname> <n> <n> <n> = "$1 $2 int $3$4/$5/$6{Enter}";
	<firstword> <secondword> into <intname> <n> <n> <n> pause =  "$1 $2 int $3$4/$5/$6";
	<mediatype> := 	( cluster | 
	                  disk 0 = disk0 | 
					  disk 1 = disk1 | 
					  flash | 
...
#	copy \/pcap cap cap <n> <mediatype> colon <mediatype> colon = copy \/pcap cap cap$1 $2: $3:";
	Control zed = {Ctrl+z};
	Control See = {Ctrl+z};
	capture capture int  = "cap cap int ?";
	capture capture rent  = "cap cap int ?";
	isdn = ' isdn ';
...
#	s s h = "ssh ";
	hoo = "who"{enter};
	delete line = Keys.SendInput({ctrl+e} {ctrl+u});
	delete word = {esc} f {ctrl+w};
	delete char = {ctrl+f} {backspace};
	delete leading char = {ctrl+a} {ctrl+f} {Backspace} {ctrl+e};
    clear leading char = {ctrl+a} {ctrl+f} {Backspace} {ctrl+e};
    fix leading char = {ctrl+a} {ctrl+f} {Backspace} {ctrl+e};
	fix leading IP address char = {esc} b {esc} b {esc} b {esc} b {Ctrl+f} {Backspace} {Ctrl+e};
	control z = {ctrl+z};
	control zed = {ctrl+z};
	control charlie = {ctrl+c};
	reload in <n> = "reload in $1{enter}";
	reload int <n> = "reload in $1{enter}";    
#<_anything>
#duplicate session = Keys.SendInput({ctrl+shift+LeftButton});	
...
temptemp | temptemp2 :
  ...
  <intnameNexus> := 'e' | 'Ethernet' = e | port channel = 'port-channel' ;
  interface <intnameNexus> <n> = "interface " $1 EvalTemplate($2);
  interface <intnameNexus> <n> <n> = "interface " $1 EvalTemplate($2) "/" EvalTemplate($3);
  interface <intnameNexus> <n> <n> <n> = "interface " $1 EvalTemplate($2) "/" EvalTemplate($3) "/" EvalTemplate($4);
  interface <intnameNexus> <n> <n> <n> slash <n> slash <n> = "interface " $1 EvalTemplate($2) EvalTemplate($3) EvalTemplate($4) "/" EvalTemplate($5) "/" EvalTemplate($6say that);
  interface <intnameNexus> <n> <n> <n> slash <n> slash <n> dash <n> = "interface " $1 EvalTemplate($2) EvalTemplate($3) EvalTemplate($4) "/" EvalTemplate($5) "/" EvalTemplate($6) "-" EvalTemplate($7);
	undebug = 'undebug ';
	debug = 'debug ';
	disable - all = 'disable-all';
...
#	trace route = "traceroute ";
	hoo = "who"{enter};
	delete line = Keys.SendInput({ctrl+a} {ctrl+k});
	delete word = {esc} f {ctrl+w};
	delete char = {ctrl+f} {backspace};
	delete leading char = {ctrl+a} {ctrl+d} {ctrl+e};
    clear leading char = {ctrl+a} {ctrl+d} {ctrl+e};
    fix leading char = {ctrl+a} {ctrl+f} {Backspace} {ctrl+e};
	fix leading IP address char = {esc} b {esc} b {esc} b {esc} b {Ctrl+f} {Backspace} {Ctrl+e};
	control z = {ctrl+z};
	control zed = {ctrl+z};
...
<nfirstword> :=	(	attach | BF shell = bfshell | blink = blink | blogger D = bloggerd | call home = callhome | CD = cd | CFS = cfs | change password = change-password | checkpoint = checkpoint | clear = clear | CLI = cli | clock | configure = configure | cop = copp | copy | crypto | debug | debug filter = debug-filter | delete | diagnostic | iff clean = diff-clean | dIR = dir | DOS to NX OS = dos2nxos | echo | EEM test  = eemtest | encryption | Ethan analyzer = ethanalyzer | event | fabric | FEX = fex | find | format | F trace = ftrace | generate | guests shall = guestshell | GM zip = gunzip | gzip | hardware | install | IP = ip | key | kill | L3 VM = l3vm | load | locator LED = locator-led | maker = mkdir | make their = mkdir | mount | move | M trace = mtrace | nTP = ntp | perfect = perf | personality | ping | ping 6 = ping6 | packet manager = pktmgr | poll period = poll-period | priority flow control = priority-flow-control | PWG = pwd | Python = python | QoS = qos | reload | restart | RM Dürer = rmdir | rollback | routing context = routing-context | run | run script = run-script | scripting | send | session | set | set up = setup | show | sleep | source | slot | snapshot | sockets | ssh | ssh 6 = ssh6 | switch back = switchback | switch to = switchto | system | ack back = tac-pac | Tahoe Python = tahoe-python | tail | tar | TCL shell = tclsh | Telnet = telnet | Telnet 6 = telnet6 | terminal | test | trace route = traceroute | trace route 6 = traceroute6 | trap | trigger | undo but = undebug | undebug | unmount | update | virtual service = virtual-service | vpc | watch | write | xML  = xml | XML in = xmlin | end | exit | pop | push | where );   
...
#   interface ethernet 101/1/8
#   interface ethernet 101/1/8-9
#Keystrokes
#Description
...
#Down arrow key	Displays the next command in the command history.Right arrow key
#Left arrow key	Moves your cursor through the command string, either forward or backward, allowing you to edit the current command.
